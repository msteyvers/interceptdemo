...
let NUM_OBJECTS = 15; // Number of animated objects
let AI_HELP = 1; 
...
function initializeObjects() {
    ...
    for (let i = 0; i < NUM_OBJECTS; i++) {
        ...
        do {
            ...
            const finalradius = Math.sqrt( (finalx-centerX)**2 + (finaly-centerY)**2 );
...
function animateInterception() {
    ...
    let isInCircle = Math.sqrt( (player.x - centerX)**2 + (player.y - centerY)**2 ) <= GAME_RADIUS;
...
function handleMouseHover(event) {
    ...
    hoverObjectIndex = objects.findIndex(
        (object) => Math.hypot(mouseX - object.x, mouseY - object.y) <= object.radius
    );
...
function handleObjectSelection(event) {
    ...
    for (let object of objects) {
        ...
        if (distance <= object.radius && !object.isSelected && selectedObjects.length < NUM_SELECTIONS) {
        ...
        if (!object.isIntercepted) {
            ...
            */
            ...
            if (object.isSelected) {
                ...
                ctx.fillStyle = 'black';
                ctx.font = '24px Arial';
                ctx.fillText(selectionIndex + 1, object.x + object.radius + 14, object.y + 8);
            }

            if ((canShowAISolution)) {
                let AISelectionIndex = bestSolution.sequence.indexOf( index );
                if (AISelectionIndex  !== -1) {
                    ctx.fillStyle = 'blue';
                    ctx.font = '24px Arial';
                    ctx.fillText(AISelectionIndex + 1, object.x - object.radius - 20, object.y + 8);
...
playerImage.src = './player.png'; // Ensure this path matches your file structure

function drawPlayer() {
    // Draw the player image
    if (playerImage.complete) { // Ensure the image is loaded before drawing
        const imageWidth  = 60; // Adjust the size of the image
        const imageHeight = 60;
        ctx.drawImage(playerImage, player.x - imageWidth/2, player.y - imageHeight/2, imageWidth, imageHeight);
    } else {
    ...
    if (!true) {
        if (playerSolution) {
           let currentMove = playerSolution.moves[ interceptionCounter ]; // object that contains all information for intercepting the current object
           ctx.beginPath();
           ctx.arc(currentMove.interceptPosX, currentMove.interceptPosY, 15, 0, Math.PI * 2);
           ctx.fillStyle = 'blue';
           ctx.fill();
        }
    }
...
function findMatchingPermutationIndex(permutations, selectedObjects) {
    return permutations.findIndex(permutation => 
...
function lookupInterceptionPaths() {
    ...
    const matchingIndex = findMatchingPermutationIndex(permutations, selectedObjects);
    let playerSolution;
...
function enumerateAllSolutions() {
    ...
    for (i=0; i<numSequences; i++) {
        ...
        for (j=0; j<NUM_SELECTIONS; j++) {
            ...
            let [success, withinCircle, timeToIntercept, interceptPosX, interceptPosY, distance] = attemptIntercept(
                copyPlayer.x, copyPlayer.y, copyPlayer.speed,
                objectNow.x, objectNow.y, objectNow.dX, objectNow.dY );

            
            let move = { success: success, withinCircle: withinCircle, value: 0 };
            if (success) { // interception is possible (within or outside the circle)
                if (withinCircle) {
...
function attemptIntercept(playerPosX, playerPosY, playerSpeed, 
                          objectPosX, objectPosY, objectVelX, objectVelY ) {
    let success = false;
    ...
    if (Math.sqrt((interceptPosX-centerX) ** 2 + (interceptPosY-centerY) ** 2) <= GAME_RADIUS) {
        withinCircle = true;
    }
